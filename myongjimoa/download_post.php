<?php
	...
	$sql = "SELECT `board`.`id`, `board`.`title`, `board`.`description`, `user`.`number`, `user`.`major`, `board`.`date`, `user`.`nickname` FROM `board` LEFT JOIN `user` ON `board`.`user_id`=`user`.`id` WHERE `board`.`board_title_id`=".$board_title_id." AND `board`.`id`<$count_board_id ORDER BY `board`.`id` DESC LIMIT 15";
